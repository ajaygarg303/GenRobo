...
async def ensure_phase1_schema() -> None:
    ...
    async with engine.begin() as conn:
        if backend == "postgresql":
            ...
            await _pg_add_column(conn, "chat_sessions", "visitor_email", "VARCHAR(200)")
            await _pg_add_column(conn, "chat_sessions", "visitor_phone", "VARCHAR(64)")
            await _pg_add_column(conn, "chat_sessions", "visitor_name", "VARCHAR(120)")
            ...
            await _sqlite_add_column(conn, "chat_sessions", "visitor_email", "VARCHAR(200)")
            await _sqlite_add_column(conn, "chat_sessions", "visitor_phone", "VARCHAR(64)")
            await _sqlite_add_column(conn, "chat_sessions", "visitor_name", "VARCHAR(120)")
